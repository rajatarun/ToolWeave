...
Parses the spec and writes all endpoint entries to DynamoDB (ApiMetaTable
and ApiCatalogTable), replacing any previous data for the same S3 key.
"""
...
def lambda_handler(event: dict[str, Any], context: Any) -> dict[str, Any]:  # noqa: ARG001
    ...
    for record in event.get("Records", []):
        ...
        try:
            ...
            logger.error("Failed to process s3://%s/%s: %s", bucket, key, exc, exc_info=True)
            errors += 1
...
def _process_file(bucket: str, key: str) -> None:
    ...
    # Derive context_name from api_title (normalised)
    import re
    context_name = re.sub(r"[^a-zA-Z0-9]+", "", api_title)

    # Delete existing entries for this key (idempotent update)
