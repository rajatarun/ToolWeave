...
def _entry_to_item(entry: EndpointEntry, api_id: str) -> dict[str, Any]:
    return {
        ...
        # DynamoDB attribute is `sample_prompts` to match table schema.
        # The in-memory model continues to use `example_prompts` (Bedrock output field).
        "sample_prompts": json.dumps(entry.example_prompts),
        "parameter_notes": json.dumps(entry.parameter_notes),
...
def _item_to_entry(item: dict[str, Any]) -> EndpointEntry:
    ...
    return EndpointEntry(
        ...
        example_prompts=json.loads(
            item.get("sample_prompts") or item.get("example_prompts") or "[]"
        ),
        parameter_notes=json.loads(item.get("parameter_notes") or "{}"),
